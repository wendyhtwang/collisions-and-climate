...
- precip_mm/snowfall_mm are summed (monthly total, same convention as
  PRISM's ppt); tmean_c, tmin_c, tmax_c, dewpoint_c, skin_temp_c,
  wind_speed_10m, snow_depth, surface_pressure are averaged. snow_depth is
  a stock (snow currently on the ground), not a flux, so a monthly mean is
  the meaningful summary, not a sum.
- No dataset-vintage flag here: unlike PRISM (AN81/AN91), ERA5-Land is a
  single reanalysis product with no vintage boundary in this period, and
  04_extract_era5_county.py doesn't emit a dataset_type column.
...
- Drops confirmed byte-identical duplicate rows automatically (same known
  18-WI-county TIGER duplicate-feature case documented for PRISM in
  SCRIPT_OVERVIEW.md -- the ERA5 extraction draws counties from the same
  TIGER/2018/Counties source, so it's plausible here too); raises an error
  instead if duplicate rows actually disagree in value, since that needs a
  human look.
...
# era5_county_daily_<year>_<run_timestamp>.csv
# Note: the personal dev repo fallback also holds small-scale test files
# (era5_county_daily_IN_2020_*.csv, era5_county_daily_IL_2021_*.csv, etc.)
# directly alongside the full-run files rather than in their own
# subfolder. check_for_year_conflicts() below deliberately raises on those
# rather than silently including or excluding them, since they don't match
# the <year> naming convention -- move/remove them from that folder before
# aggregating from the fallback path.
...
# without holding all years of daily rows in memory at once.
# ---------------------------------------------------------------------

# Same duplicate-row shape as the PRISM aggregation (18 WI counties with a
# source-data duplicate county feature -- see SCRIPT_OVERVIEW.md). Kept
# generic/dataset-agnostic since it's not yet confirmed whether ERA5 hits
# the exact same GEOIDs.
