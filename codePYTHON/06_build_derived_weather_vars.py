...
- Same QA checks as aggregation scripts (05/05b), via aggregation_utils.py:
...
def compute_month_derived_vars(daily: pd.DataFrame, config: DatasetConfig) -> pd.DataFrame:
    ...
    # matches 05/05b's missing-data convention (also applied independently
...
def main() -> None:
    ...
    print(
        ...
        "these as final (see module docstring for detail): the "
        f"heating/cooling degree-day base temperature "
        f"({HDD_CDD_BASE_F}F / {HDD_CDD_BASE_C:.2f}C)."
    )
