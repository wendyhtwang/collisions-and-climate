...
FULL-SCALE CHANGES from the prototype (validated against a 2-state, 2-year
test sample):
1. All 7 PRISM bands are aggregated, not just ppt/tmean/tmin/tmax -- the
   full extraction (02_extract_prism_county.py) pulls tdmean, vpdmin, and
   vpdmax too. Averaging is assumed appropriate for all three (same
   treatment as tmean/tmin/tmax); flag if a different treatment is wanted
   for the data dictionary.
2. Each input file is loaded and aggregated to monthly one at a time,
   rather than concatenating all ~46 daily files (~9 GB combined) into one
   DataFrame before grouping. Each file is one full calendar year, so a
   month's data is never split across files -- aggregating file-by-file
   and concatenating the (much smaller) monthly results is equivalent to
   aggregating the full concatenated daily table, just without holding all
   46 years of daily rows in memory at once.
3. INPUT_DIR resolves across machines (Kodama vs. personal dev repo),
   matching the candidate-path convention used in 02/03's
   MOVE_DESTINATION_CANDIDATES -- the finalized full run's 46 CSVs live on
   Kodama, not in this repo's dataRAW/PRISM/. (Duplicated here rather than
   importing gee_extract_utils.resolve_data_root() so this pure
   pandas/QC script doesn't pull in the earthengine-api dependency.)
4. Duplicate/overlap detection is now two-layered: within a file (same
   geoid+date appearing twice -- e.g. an accidental double export) and
   across files (two files claiming the same year -- e.g. a rerun that
   produced a second CSV for a year already extracted). The latter can't
   be caught by a within-file check once aggregation happens per-file, so
   it's checked explicitly before any aggregation runs.
5. Added a completeness check: flags any county-month whose day count
   doesn't match the calendar days expected for that month/year (leap
   years included), instead of silently averaging/summing over
   partial-month data.
...
# Aggregation treatment per PRISM band: precipitation is a monthly total,
...
# ---------------------------------------------------------------------
# Load + aggregate (one file at a time)
# ---------------------------------------------------------------------
...
def load_daily_extract(path: Path) -> pd.DataFrame:
    ...
    if duplicate_mask.any():
        n_dupes = daily.loc[duplicate_mask, ["geoid", "date"]].drop_duplicates().shape[0]
        raise ValueError(
            f"{path.name}: found {n_dupes} geoid/date combinations "
            "duplicated within this file -- check the export before aggregating."
        )
...
def main() -> None:
    ...
    for i, path in enumerate(paths, start=1):
        ...
        del daily  # free the daily rows before loading the next file
